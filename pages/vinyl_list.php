...
// ดึงข้อมูลป้ายไวนิล
$vinyl_boards = $conn->query("SELECT * FROM sign_boards WHERE type = 'Vinyl'");
...
<div class="container py-4">
    ...
    <div class="card p-4 shadow-lg" style="border-radius: 12px; background-color: #ffffff;">
        ...
        <div class="row row-cols-1 row-cols-md-2 g-4 mb-4">
            <?php while ($row = $vinyl_boards->fetch_assoc()) echo renderSignCard($row); ?>
        </div>

        <div class="row mb-4">
            <div class="col">
                <?php include('../components/vinyl_schedule.php'); ?>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title" id="pdfModalLabel">แนวปฏิบัติการใช้งานป้ายไวนิล</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                <iframe src="../documents/guidelines_vinyl.pdf#zoom=58" width="100%" height="600px" style="border:none;"></iframe>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.modal').forEach(function(modal) {
        // เฉพาะ modal ที่ใช้จอง (มี bookingModal ใน ID)
        if (modal.id.includes('bookingModal')) {
    ...
    const pdfModalKey = 'pdfModalShown_vinyl';
            ...
            <div class="modal-content">
                <form action="../submit_booking.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body bg-light">
                        <div class="mb-4">
                            <h6 class="mb-2"><i class="bi bi-calendar-event"></i> ปฏิทินแสดงกิจกรรมการจองของป้ายนี้</h6>
                            <iframe src="../components/vinyl_calendar_single.php?sign_board_id=<?= $row['id'] ?>" style="width:100%; height:500px; border:none; border-radius: 10px;" loading="lazy"></iframe>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6"><label class="form-label">วันที่เริ่ม</label><input type="date" name="start_date" class="form-control" required></div>
                            <div class="col-md-6"><label class="form-label">วันที่สิ้นสุด</label><input type="date" name="end_date" class="form-control" required></div>
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">อัปโหลดภาพ (.jpg, .png)</label>
                            <input type="file" name="sample_file" class="form-control file-input" accept=".jpg,.jpeg,.png">
                            <div class="form-text">อนุญาตเฉพาะ .jpg, .png ขนาดไม่เกิน 10MB</div>
                            <div class="error-message" style="color: red; display: none; margin-top: 5px;">ขนาดไฟล์เกิน 10MB</div>
                        </div>
                    </div>

                    <div class="modal-footer">
                        <input type="hidden" name="sign_type" value="Vinyl">
